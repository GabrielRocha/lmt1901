...
function build_table(selector, url){
    ...
    $(selector+' tbody').on('click', 'input[type="checkbox"]', function(e){
      ...
      var index = rowId in rows_selected;

      // If checkbox is checked and row ID is not in list of selected row IDs
      if(this.checked && index === false){
          month_values = $row.find("td").slice(2);
          var str = [];
          $.each(month_values, function(data){str.push($(this).text())});
          rows_selected[rowId] = str;

      // Otherwise, if checkbox is not checked and row ID is in list of selected row IDs
      } else if (!this.checked && index !== false){
